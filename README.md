...
Build 5.18 makes the web app lighter and calmer: the queue can load the full matching album list, idle polling slows down, row rendering avoids unnecessary rebuilds, Settings now has Auto/Light/Dark theme modes, and the Summary panel is reduced to a short status line plus compact facts. The next migration step is adding manual image import, backup/restore browsing, diagnostics export, and deeper provider controls.
